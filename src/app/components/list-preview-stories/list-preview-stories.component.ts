...
export class ListPreviewStoriesComponent implements OnInit {
  ...
  async presentModal(item: {
    ...
    this.oemLoggingService.registerEvent('story_clic', convertObjectToLoggingPayload({ msisdn: this.currentMsisdn }));
    ...
    modal.onDidDismiss().then(res => {
      if (res.role) {
        this.oemLoggingService.registerEvent('story_close', convertObjectToLoggingPayload({ msisdn: this.currentMsisdn }));
      }
      if (res?.data) {
