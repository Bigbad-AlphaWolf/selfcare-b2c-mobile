...
import { convertObjectToLoggingPayload, getCountryInfos, replaceWhiteSpaceWithCaracter } from '../utils/utils';
...
export class NewSelectBeneficiaryPage implements OnInit {
    ...
    await from(this.diagnostic.getContactsAuthorizationStatus()).pipe(
      tap(contactStatus => {
        ...
        if (contactStatus === this.diagnostic.permissionStatus.NOT_REQUESTED || contactStatus === this.diagnostic.permissionStatus.DENIED_ALWAYS) {
          ...
          this.errorMsg = "Pour afficher vos contacts/bénéficiaires récents, vous devez autoriser l'accés à vos contacts.";
        }
            ...
            if (item.numbers.length > 1) {
              ...
              const contacts: ContactOem[] = item.numbers.map(elt => {
                return {
                  ...
                  phoneNumber: elt,
                  country: getCountryInfos(elt),
                  formatedPhoneNumber: this.formatPhoneNumber(elt),
  ...
  checkTransferType(payload: any, contact: ContactOem) {
    if (contact?.country?.callId === TRANSFER_OM_INTERNATIONAL_COUNTRIES[0].callId) {
  ...
  checkRecipientHasOMAccount(payload: { senderMsisdn: string; recipientMsisdn: string; recipientFirstname: string; recipientLastname: string }) {
    const pageData = Object.assign(payload, {
  ...
  processInfosTogoToAmountPage(payload: { senderMsisdn: string; recipientMsisdn: string; recipientFirstname: string; recipientLastname: string }) {
      ...
      .pipe(
        ...
        tap((res: ContactOem[]) => {
          ...
          this.oemLoggingService.registerEvent(
            ...
            convertObjectToLoggingPayload({ msisdn: this.dashboardServ.getCurrentPhoneNumber() })
          );
        }),
        catchError(err => {
          this.loadingRecents = false;
          this.oemLoggingService.registerEvent(
            'Recuperation_destinataire_transfert_om_recents_failed',
