...
export class OfcService {
  ...
  loadOFC(cookies: string[] = null) {
    ...
    var browser = this.inAppBrowser.create(OfcService.OFC_WEBVIEW_URL, '_blank', `location=no,hideurlbar=yes`);
  ...
  onStopLoad(browser, cookies) {
    browser.executeScript(
      ...
      params => {
        ...
        this.ofcLoadTimes = !cookies && this.sessionExpired(inBrowserCookiesStr) ? 1 : this.ofcLoadTimes;
  ...
  setCookies(browser, cookies) {
    ...
    const cookieCode = 'document.cookie="' + cookieName + '=' + cookieValueArr[0] + '";';
    browser.executeScript({
      code: cookieCode
    });
    ...
    const expiresDateCode = 'document.cookie="expires_date=' + expireDateStr + '";';
    browser.executeScript({
