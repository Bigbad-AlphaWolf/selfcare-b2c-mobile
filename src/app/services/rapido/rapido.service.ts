...
import {
  RAPIDO_PAYMENT_ENDPOINT,
  RAPIDO_SOLDE_ENDPOINT,
...
export class RapidoService {
  ...
  pay(body: any, confirmPayload?) {
    const queryParams = confirmPayload?.txnId
      ? `?txnId=${confirmPayload?.txnId}`
      : '';
    return this.http.post(`${RAPIDO_PAYMENT_ENDPOINT}${queryParams}`, body);
  }
      ...
      switchMap((msisdn: string) => {
        if (msisdn !== 'error') {
          return this.http.get(
            RAPIDO_SOLDE_ENDPOINT + `/${msisdn}?cardNum=${counterNumber}`
          );
        } else {
          const error = {
            status: 400,
