...
export class BillsService {
	...
	invoices(
		...
		return this.http
			.get<InvoiceOrange[]>(
				`${INVOICE_ENDPOINT}/${codeClient}?type=${type}&search=${type === 'MOBILE' ? 'phoneNumber:' + phone + ',' : ''
	...
	async moisDisponible( codeClient: string, type: string, phone?: string ) {
		return await this.http
			.get<string>(
				`${INVOICE_MOIS_DISPO_ENDPOINT}/${codeClient}?type=${type}${type === 'MOBILE' ? '&search=phoneNumber:' + phone : ''
				}`
			)
			.pipe(
				catchError( ( err ) => {
