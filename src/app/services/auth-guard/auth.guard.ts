...
export class AuthGuard implements CanActivate {
  ...
  constructor(private authServ: AuthenticationService, private router: Router, private dashboardService: DashboardService) {}
  canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<boolean> | boolean {
    ...
    if (!userHasLogin) {
      ...
      if (state.url === '/dashboard/all-my-conso/transfert-credit' || state.url === '/transfer/credit-pass') {
        this.dashboardService.getUserConsoInfosByCode(null, [1, 2, 6]).subscribe((res: any) => {
