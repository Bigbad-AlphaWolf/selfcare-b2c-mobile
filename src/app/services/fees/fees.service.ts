...
export class FeesService {
  fees: any;
  feesIncludes: any;
  constructor(private http: HttpClient, private orangeMoneyService: OrangeMoneyService) {}
    ...
    return this.orangeMoneyService.getOmMsisdn().pipe(
      switchMap((omMsisdn: string) => {
        ...
        return this.http.get(`${FEES_ENDPOINT}/${receiver}?msisdn=${omMsisdn}&service-code=${service}`).pipe(
          map((fees: FeeModel[]) => {
            if (service === OM_LABEL_SERVICES.TAF || service === OM_LABEL_SERVICES.TRANSFERT_AVEC_CODE) {
              fees = fees.map(fee => {
  ...
  extractSendingFeesTransfertOM(fees: FeeModel[], amount: number) {
    for (let fee of fees) {
      if (amount >= +fee.min && amount <= +fee.max) {
        const effective_fees = fee.mode_calcul === 'fixe' ? fee.effective_fees : Math.round(amount * fee.effective_fees) / 100;
        const old_fees = fee.old_mode_calcul === 'fixe' ? fee.old_fees : Math.round(amount * fee.old_fees) / 100;
        const cashout_fees = fee.mode_calcul === 'fixe' ? fee.cashout_fees : Math.round(amount * fee.cashout_fees) / 100;
