...
export class PassIllimixService {
  ...
  constructor(private dashbService: DashboardService, private http: HttpClient) {}

  setUserCodeFormule(msisdn: string) {
    this.userCodeFormule = msisdn;
  }

  queryListPassIllimix(codeFormule: string, recipientPass: string, category?: string, isLightMod?: boolean) {
    this.listPassIllimix = [];
    return this.dashbService.getListPassIllimix(codeFormule, category, isLightMod).pipe(
      switchMap(async (res: any[]) => {
        const response: any = await this.dashbService.getListPassByMsisdn('ILLIMIX', recipientPass).toPromise();
      ...
      map((res: PassIllimixModel[]) => {
        res.forEach(x => {
          const isPassAllo =
            (x.pass && x.pass.categoriePass.libelle.toLowerCase() === ALLO_PASS_CATEGORY.toLowerCase()) ||
            (x.promoPass && x.promoPass.passPromo.categoriePass.libelle.toLowerCase() !== ALLO_PASS_CATEGORY.toLowerCase());
        ...
        if (!category) {
          list = list.filter(category => {
            return category.libelle.toLowerCase() !== ALLO_PASS_CATEGORY.toLowerCase();
          });
        }
        this.listCategoryPass = getOrderedListCategory(list);
        this.listPassIllimixShown = getListPassFilteredByLabelAndPaymentMod(this.listCategoryPass[0], this.listPassIllimix);
        return list;
      })
    );
  }
