    ...
    this.emergencyService.getCodeCarteRecharge(carteInfos).subscribe(
      res => {
        ...
        if (res === 'consecutif') {
          ...
          this.followAnalyticsService.registerEventFollow('Code_Carte_Recharge_Error', 'error', this.errorFiveConsecutiveNumbers);
        } else if (res === 'NumeroSerie') {
          this.errorNumSerie = 'Le numéro de série saisi est incorrect';
          this.followAnalyticsService.registerEventFollow('Code_Carte_Recharge_Error', 'error', this.errorNumSerie);
        } else if (res === 'OK') {
          this.openSuccessDialog('carte');
          this.followAnalyticsService.registerEventFollow('Code_Carte_Recharge_Success', 'event', msisdn);
        } else {
