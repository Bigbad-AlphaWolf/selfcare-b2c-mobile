...
export class FixesServicesPage implements OnInit, AfterViewInit {
  ...
  getServices() {
    ...
    this.operationService
      .getServicesByFormule(HUB_ACTIONS.FIXES)
      .pipe(
        tap(res => {
          console.log('res', res);
          ...
          const followEvent = 'Get_hub_offres_fixe_services_success';
          this.oemLoggingService.registerEvent(followEvent, convertObjectToLoggingPayload({ msisdn: this.phone }));
        }),
        catchError(err => {
          this.loadingServices = false;
          this.errorMsg = 'Veuillez réessayer dans quelques minutes. Nous avons des soucis à récuperer les infos';
          const followError = 'Get_hub_offres_fixes_services_failed';
