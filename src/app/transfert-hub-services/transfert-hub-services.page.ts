...
export class TransfertHubServicesPage implements OnInit {
	...
	buyOptions: {
		...
		subtitle: string;
		icon: string;
		type: 'CREDIT' | 'PASS' | 'PASS_ILLIMIX' | 'PASS_VOYAGE' | 'PASS_INTERNATIONAL' | 'PASS_ALLO';
		url?: string;
		action?: 'REDIRECT' | 'POPUP';
		idCode?: number;
	}[] = [
	...
	ngOnInit() {
		...
		if (history && history.state) {
			...
			if (!this.isLightMod) {
				...
				if (this.isServciceActivated(this.buyIlliflexOption)) this.buyOptions.push(this.buyIlliflexOption);
			}
		}
	...
	isServciceActivated(action) {
		const actionService = OperationService.AllOffers.find((service) => action.idCode && service.code === action.idCode);
	...
	async goTo(opt: {
		...
		if (!this.isServciceActivated(opt)) {
			const service = OperationService.AllOffers.find((service) => opt.idCode && service.code === opt.idCode);
