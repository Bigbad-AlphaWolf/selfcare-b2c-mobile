...
import {
  ...
  OPERATION_TYPE_MERCHANT_PAYMENT} from 'src/shared';
...
export class TransfertHubServicesPage implements OnInit {
  ...
  openModalPassNumberSelection(operation: string, routePath: string) {
    if (this.isLightMod) {
      ...
      this.bsService.openNumberSelectionBottomSheet(NumberSelectionOption.WITH_MY_PHONES, operation, routePath, this.isLightMod);
    }
  }
    ...
    return this.favService
      ...
      .pipe(
        tap((res: any) => {
          ...
          this.followAnalyticsService.registerEventFollow('Get_favorite_pass_success', 'event');
        }),
        catchError(err => {
          this.followAnalyticsService.registerEventFollow('Get_favorite_pass_error', 'error', {
            msisdn: this.currentPhone,
            error: err.status
          });
          return of(null);
  ...
  choosePass(pass: any, opType: string) {
    this.followAnalyticsService.registerEventFollow('Select_favorite_pass', 'event', this.currentPhone);
