...
export class SatisfactionFormPage implements OnInit {
  ...
  constructor(
    ...
    private satisformServ: FormulaireSatisfactionService,
  ...
  fetchQuestions() {
    ...
    this.satisformServ
      .queryQuestions()
      .pipe(
        tap((res: QuestionFormSatisfactionModel[]) => {
  ...
  isFormValid() {
    let isValid = false;
    const QUESTIONS_NOT_FILLED = this.listAnswer.filter(
      (answer: {feedback: any; comment: any; questions: QuestionFormSatisfactionModel}) => {
