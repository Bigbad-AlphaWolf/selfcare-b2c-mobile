...
export class ItemQuestionSatisfactionFormComponent implements OnInit {
	...
	isType( question: QuestionFormSatisfactionModel, typeQuestion: string ) {
		return question && question.typeQuestion === typeQuestion;
	}
	...
	onAnswerSelectedFromSelection( input: any ) {
		...
		if ( input.detail.value ) {
			if ( this.itemQuestion && this.itemQuestion.libelle === 'multi' ) {
	...
	initializeAllNotes() {
		if ( this.itemQuestion && this.itemQuestion.libelle && !isNaN( +this.itemQuestion.libelle ) ) {
