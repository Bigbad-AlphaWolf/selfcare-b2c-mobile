...
export class SelectBeneficiaryV2Page implements OnInit {
  ...
  purchaseType: string;
  constructor(
    private dashbbServ: DashboardService,
  ...
  getListeRattachedNumber() {
    this.isLoaded = false;
    this.listUserNumber = [];
    this.dashbbServ.getAttachedNumbers().subscribe(
      (res: any[]) => {
        this.isLoaded = true;
        this.hasError = false;
        if (res.length) {
  ...
  getContactFormattedName(contact: any) {
    ...
    const middleName = contact.name.middleName
  ...
  processInfoUser() {
    ...
    if (this.isUserRecipient) {
      ...
      if (
        this.ionicInput &&
        this.ionicInput.value &&
        this.validateNumber(this.ionicInput.value)
      ) {
      ...
      this.authServ.getSubscription(this.recipientNumber).subscribe(
        (res: SubscriptionModel) => {
          ...
          if (
            ...
            const data = {
              destinataire: this.recipientNumber,
              code: codeFormule,
              recipientName: this.recipientContactInfos,
            };
