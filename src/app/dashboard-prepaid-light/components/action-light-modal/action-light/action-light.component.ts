  ...
  login() {
    ...
    this.authServ.login(user).subscribe(
      () => {
        ...
        this.oemLoggingService.registerEvent('login_success', convertObjectToLoggingPayload(user.username));
      ...
      err => {
        this.oemLoggingService.registerEvent('login_failed', convertObjectToLoggingPayload(user.username));
        this.loading = false;
