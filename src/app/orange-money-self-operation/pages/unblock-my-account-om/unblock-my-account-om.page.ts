...
export class UnblockMyAccountOmPage implements OnInit {
  ...
  setValue(typeInfos: 'nom' | 'prenom' | 'date_naissance' | 'cni' | 'conditions', valueIndex: any) {
    ...
    const nullItemValue: { label: string; value: string; code: string } = this.listItem.find(
      (res: { label: string; value: string; code: string; listChoices: string[] }) => !res.value
    );
  ...
  fetchOMChallenge() {
    ...
    this.omService
      .fetchOMChallengeForUnlockAndReset(this.omMsisdn)
      .pipe(
