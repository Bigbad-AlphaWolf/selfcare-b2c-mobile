...
import {
  ERROR_MSG_OM_CODE_OTP_INVALIDE,
  SUCCESS_OM_STATUS_CODE,
...
export class TypeOtpModalComponent implements OnInit, AfterViewInit {
  ...
  sendCode() {
    ...
    this.orangeMoneyService
      .initSelfOperationOtp(this.checkOtpPayload.kyc)
      .subscribe(
        (_) => {
  ...
  checkOtp() {
    ...
    const eventTypeName =
      this.checkOtpPayload.typeDemande === 'INSCRIPTION'
        ? 'open_om_account'
        : 'deplafonnement_om';
    this.orangeMoneyService
      ...
      .subscribe(
        ...
        (err) => {
          ...
          if (
            err &&
            err.status === 400 &&
            err.error.code === 'OTP_CHECK_ERROR'
          ) {
