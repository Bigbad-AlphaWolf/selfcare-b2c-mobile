...
import { OPERATION_RAPIDO, OPERATION_TYPE_PASS_USAGE, OPERATION_WOYOFAL } from '../utils/operations.constants';
...
export class OperationRecapPage implements OnInit {
        ...
        if (this.router.getCurrentNavigation()) {
          ...
          switch (this.purchaseType) {
            ...
            case OPERATION_TRANSFER_OM:
              ...
              this.recipientName = this.opXtras.recipientFirstname + ' ' + this.opXtras.recipientLastname;
              this.paymentMod = PAYMENT_MOD_OM;
              break;
    ...
    this.dalalTonesService.activateDalal(this.opXtras.dalal).subscribe(
      ...
      err => {
        ...
        const activationErrorMsg =
          err && err.error && err.error.message ? err.error.message : 'Une erreur est survenue';
  ...
  async setPaymentMod() {
    ...
    modal.onDidDismiss().then(response => {
      let eventName =
        this.purchaseType === OPERATION_TYPE_PASS_ILLIFLEX ? 'Buy_illiflex_payment_mod' : 'Buy_pass_payment_mod';
  ...
  get operationTypeRecap() {
    return ['RECHARGEMENT_CREDIT', OPERATION_TYPE_PASS_VOYAGE, 'OPERATION_WOYOFAL', OPERATION_RAPIDO].includes(
      this.purchaseType
    );
  }
