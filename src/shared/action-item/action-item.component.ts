...
import { OPERATION_RAPIDO, OPERATION_XEWEUL, OPERATION_TYPE_PASS_USAGE, OPERATION_WOYOFAL } from 'src/app/utils/operations.constants';
import {
...
export class ActionItemComponent implements OnInit {
  ...
  doAction() {
    ...
    if (this.action.passUsage) {
      this.bsService.openNumberSelectionBottomSheet(NumberSelectionOption.WITH_MY_PHONES, OPERATION_TYPE_PASS_USAGE, 'list-pass-usage', this.action);
      return;
    }
    ...
    if (this.bsService[this.action.redirectionType]) {
      const params = [NumberSelectionOption.WITH_MY_PHONES, this.action.code, this.action.redirectionPath];
    ...
    switch (this.action.code) {
      ...
      case OPERATION_TYPE_SEDDO_CREDIT:
        ...
        break;
      case 'FIBRE_OPTIC':
        this.oemLoggingService.registerEvent('help_fibre_eligibility_click', []);
      ...
      case 'SET_INTERNET':
        ...
        break;
      case 'SEDDO_CODE':
        this.oemLoggingService.registerEvent('help_change_seddo_code_click', []);
        this.goChangeSeddo();
        break;
      case 'SUIVI_DEMANDE':
      ...
      case 'TRANSACTION_ERROR':
        this.oemLoggingService.registerEvent('help_erreur_transaction_om_click', []);
        this.goReclamation();
        break;
      ...
      case 'KIOSK_LOCATOR':
        ...
        break;
      case 'BLOCK_TRANSFER':
        this.oemLoggingService.registerEvent('help_block_transfer_om_click', []);
      ...
      default:
        if (this.action?.redirectionType === 'NAVIGATE') {
          this.oemLoggingService.registerEvent('Offre_service_' + this.action?.code + '_clic');
          this.navController.navigateForward([this.action.redirectionPath], {
            state: { purchaseType: this.action?.code },
          });
        }
  ...
  async openKioskLocatorModal() {
    this.oemLoggingService.registerEvent('Kiosk_locator_clic');
    const modal = await this.modalController.create({
  ...
  async openBlockTransferModal() {
    this.oemLoggingService.registerEvent('block_transfer_clic');
    const modal = await this.modalController.create({
  ...
  goToUnblockOMAccount() {
    ...
    this.oemLoggingService.registerEvent('Assistance_actions_deblocage_compte_om_clic');
  }

  goToResetOMAccount() {
    this.router.navigate(['/om-self-operation/unblock-om-account/reset-pin']);
    this.oemLoggingService.registerEvent('Assistance_actions_reset_pin_compte_om_clic');
  }

  goFiberEligibility() {
    this.inAppBrowser.create(CHECK_ELIGIBILITY_EXTERNAL_URL, '_self');
    this.oemLoggingService.registerEvent('Assistance_actions_eligibilité_fibre_clic');
  }

  goPuk() {
    this.router.navigate(['/control-center/puk']);
    this.oemLoggingService.registerEvent('Assistance_actions_Find_PUK_clic');
  }

  goChangeSeddo() {
    this.router.navigate(['/control-center/change-seddo-code']);
    this.oemLoggingService.registerEvent('Assistance_actions_Change_Seddo_PIN_clic');
  }

  goInternet() {
    this.router.navigate(['/control-center/internet-mobile']);
    this.oemLoggingService.registerEvent('Assistance_actions_Parametrage_Internet_Mobile_clic');
  }
  goCreateOMAccount() {
    this.router.navigate(['/om-self-operation/open-om-account']);
    // this.router.navigate(['/control-center/operation-om/creation-compte']);
    this.oemLoggingService.registerEvent('Assistance_actions_New_Creation_Compte_OM_clic');
  }

  goCreateOMAccountOldVersion() {
    this.router.navigate(['/control-center/operation-om/creation-compte']);
    this.oemLoggingService.registerEvent('Assistance_actions_Creation_Compte_OM_clic');
  }

  goDeplafonnement() {
    // this.router.navigate(['/control-center/operation-om/deplafonnement']);
    this.router.navigate(['/om-self-operation/deplafonnement']);
    this.oemLoggingService.registerEvent('Assistance_actions_New_Deplafonnement_OM_clic');
  }

  goDeplafonnementOldVersion() {
    this.router.navigate(['/control-center/operation-om/deplafonnement']);
    this.oemLoggingService.registerEvent('Assistance_actions_Deplafonnement_OM_clic');
  }

  goReclamation() {
    this.router.navigate(['/om-self-operation/cancel-transaction']);
    this.oemLoggingService.registerEvent('Assistance_actions_Reclamation_OM_clic');
  }

  onFollowUpRequests() {
    this.router.navigate(['follow-up-requests']);
    this.oemLoggingService.registerEvent('Assistance_actions_suivi_demande-fixe_clic');
  }
  ...
  async openPinpad() {
    ...
    this.oemLoggingService.registerEvent('Assistance_actions_change_pin_OM_clic');
