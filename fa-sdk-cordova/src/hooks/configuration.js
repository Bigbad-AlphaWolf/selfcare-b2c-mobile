...
module.exports.functions = {
    get_configuration_property: function (configurationJsonString, platformOS, property) {
        return get_configuration_property(configurationJsonString, platformOS, property);
    },
...
*/
function get_configuration_property(configurationJsonString, platformOS, property) {
    if (configurationJsonString) {
        const configurationJSON = JSON.parse(configurationJsonString);
        const platformJSON = configurationJSON[platformOS];
        if (platformJSON) {
            const configurationProperty = platformJSON[property];
            if (configurationProperty !== undefined) {
                return configurationProperty;
            } else {
                console.error("[FA] Could not found configuration property : \"" + property + "\" in " + get_configuration_os_path() + " file.");
            }
        } else {
